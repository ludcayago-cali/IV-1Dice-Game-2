...
const sfx = {
  ...
  botHit: document.getElementById("sfxBotHit"),
...
function setAudioLevels() {
  ...
  if (sfx.tile) sfx.tile.volume = 0.35;
  if (sfx.wrong) sfx.wrong.volume = 0.45;
  if (sfx.hit) sfx.hit.volume = 0.75;
  if (sfx.botHit) sfx.botHit.volume = 0.78;
  if (sfx.heal) sfx.heal.volume = 0.65;
...
const TILESET = {
  ...
  block: {
    crack: "assets/tile_block_crack.png",
    pit: "assets/tile_block_pit.png",
    bone: "assets/tile_block_bone.png",
  }
};
...
function buildBoard(playerSpawn, botSpawn) {
  ...
  for (const tile of blockedCandidates) {
    ...
    const blockedVariants = ["crack", "pit", "bone"];
    board[tile.row][tile.col].blockType = blockedVariants[randInt(blockedVariants.length)];
  }
  return board;
}
...
function renderTileEngine() {
  ...
  for (let row = 0; row < SIZE; row++) {
    for (let col = 0; col < SIZE; col++) {
      ...
      const tileSrc = cellData.blocked
        ? TILESET.block[cellData.blockType]
        : TILESET.ground[cellData.ground];
      ground.style.backgroundImage = `url('${tileSrc}')`;
...
async function performAttack(attacker, target) {
  ...
  if (target.team === "bot") playSfx(sfx.botHit);
