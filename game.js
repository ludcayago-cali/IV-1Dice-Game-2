...
const sfx = {
  ...
  tile: document.getElementById("sfxTileSelect"),
  wrong: document.getElementById("sfxWrongTile"),
...
function setAudioLevels() {
  ...
  if (sfx.click) sfx.click.volume = 0.34;
  if (sfx.tile) sfx.tile.volume = 0.42;
  if (sfx.wrong) sfx.wrong.volume = 0.46;
  if (sfx.hit) sfx.hit.volume = 0.78;
  if (sfx.heal) sfx.heal.volume = 0.68;
  if (sfx.lose) sfx.lose.volume = 0.82;
...
function playUiPulse(el, className, duration = 220) {
  if (!el) return;
  restartAnimationClass(el, className);
  window.setTimeout(() => el.classList.remove(className), duration);
}

function getCellEl(row, col) {
  return boardEl?.children?.[row * SIZE + col] || null;
}

function pulseTile(row, col, type = "cell-select-pop") {
  const cell = getCellEl(row, col);
  if (!cell) return;
  playUiPulse(cell, type, type === "cell-wrong" ? 280 : 220);
}

function pulseResultCard() {
  playUiPulse(resultCardEl, "sfx-pop", 260);
}

function pulseButton(el) {
  playUiPulse(el, "btn-sfx-pop", 180);
}

function triggerFrameFx(type = "hit") {
  if (!gameFrameEl) return;
  const className =
    type === "heal" ? "board-heal-flash" :
    type === "lose" ? "board-lose-shake" :
    "board-hit-flash";
  playUiPulse(gameFrameEl, className, type === "lose" ? 650 : 420);
}
...
function renderDiceFace(value) {
  ...
  if (value == null) {
    ...
    resultValueEl.classList.remove("rolling", "settled");
    return;
  }
  const safe = Math.max(1, Math.min(6, Number(value) || 1));
  resultValueEl.textContent = DICE_FACES[safe - 1];
}

async function animateDiceRoll() {
  state.locked = true;
  state.isRolling = true;
  state.rollingValue = null;
  state.dice = null;
  state.movesRemaining = 0;
  state.planningPath = [];
  state.validMoves = [];
  resultValueEl.classList.remove("settled");
  resultValueEl.classList.add("rolling");

  const finalValue = randInt(6) + 1;
  const sequence = [];
  const totalFrames = 16;

  for (let i = 0; i < totalFrames; i++) {
    sequence.push(randInt(6) + 1);
  }
  sequence.push(finalValue);

  for (let i = 0; i < sequence.length; i++) {
    state.rollingValue = sequence[i];
    renderDiceFace(state.rollingValue);
    const isLast = i === sequence.length - 1;
    const delayMs = isLast ? 80 : (45 + i * 8);
    await delay(delayMs);
  }

  resultValueEl.classList.remove("rolling");
  resultValueEl.classList.add("settled");
  pulseResultCard();

  state.dice = finalValue;
  state.rollingValue = null;
...
function renderUI() {
  ...
  if (state.isRolling && state.rollingValue != null) renderDiceFace(state.rollingValue);
  else renderDiceFace(state.dice);
  ...
  rollBtn.disabled = !state.mode || state.isRolling || !(state.phase === "roll" && state.currentTurn === "player" && !state.locked);
...
async function rollDice() {
  if (state.phase !== "roll" || state.locked || state.isRolling) return null;
  renderAll();
  return await animateDiceRoll();
}
...
function finishGame(winner) {
  ...
  stopBgm(bgmGame);
  playBgm(bgmFanfare);
  if (winner !== "player") playSfx(sfx.lose);
  triggerFrameFx(winner === "player" ? "heal" : "lose");
...
async function onTileClick(row, col) {
  if (state.currentTurn !== "player" || state.locked || state.phase !== "move") {
    playSfx(sfx.wrong);
    pulseTile(row, col, "cell-wrong");
    return;
  }
  ...
  playSfx(sfx.tile);
  pulseTile(row, col, "cell-select-pop");
...
function createLobbyModal() {
  ...
  backdrop.innerHTML = `
    <div class="action-card lobby-card">
      ...
      <div class="lobby-note" id="lobbyNote" hidden></div>
    </div>
  `;
  document.body.appendChild(backdrop);
...
document.addEventListener("click", (e) => {
  const btn = e.target.closest(".btn, .action-btn, .menu-fab");
