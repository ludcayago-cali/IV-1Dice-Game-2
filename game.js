...
function isDesignLane(row, col) {
  return row === 4 || row === 5 || col === 4 || col === 5;
}

function mirroredTile(row, col) {
  return { row: SIZE - 1 - row, col: SIZE - 1 - col };
}

function canPlaceBlock(board, row, col, playerSpawn, botSpawn) {
  if (!inBounds(row, col)) return false;
  if (board[row][col].blocked) return false;
  if (isSpawnSafeTile(row, col, playerSpawn, botSpawn)) return false;
  if (isDesignLane(row, col)) return false;
  return true;
}

function setBlock(board, row, col, blockType) {
  board[row][col].blocked = true;
  board[row][col].blockType = blockType;
}
...
function buildBoard(playerSpawn, botSpawn) {
  ...
  // Clear, readable lanes through the middle. Obstacles stay in outer quadrants.
  const smartPatterns = [
    { type: "bush",  cells: [[0,0], [1,0]] },
    { type: "crate", cells: [[0,0], [0,1]] },
    { type: "bush",  cells: [[0,0], [1,0], [1,1]] },
    { type: "crate", cells: [[0,0]] },
  ];

  const anchorCandidates = shuffle([
  ...
  for (const anchor of anchorCandidates) {
    const pattern = smartPatterns[randInt(smartPatterns.length)];
    const placements = [];

    let valid = true;
    for (const [dr, dc] of pattern.cells) {
      const r1 = anchor.row + dr;
      const c1 = anchor.col + dc;
      const mirror = mirroredTile(r1, c1);

      if (!canPlaceBlock(board, r1, c1, playerSpawn, botSpawn) ||
          !canPlaceBlock(board, mirror.row, mirror.col, playerSpawn, botSpawn)) {
        valid = false;
        break;
      }

      placements.push({ row: r1, col: c1, type: pattern.type });
      placements.push({ row: mirror.row, col: mirror.col, type: pattern.type });
    }

    if (!valid) continue;

    for (const item of placements) {
      setBlock(board, item.row, item.col, item.type);
      placedCells += 1;
    }

    if (placedCells >= BLOCK_COUNT) break;
  }

  // Light fallback: add a few mirrored singles if board is still too empty.
  const singleCandidates = shuffle([
    { row: 0, col: 3, type: "bush" },
    { row: 3, col: 0, type: "crate" },
    { row: 2, col: 0, type: "bush" },
    { row: 0, col: 2, type: "crate" }
  ]);

  for (const item of singleCandidates) {
    if (placedCells >= BLOCK_COUNT) break;
    const mirror = mirroredTile(item.row, item.col);
    if (!canPlaceBlock(board, item.row, item.col, playerSpawn, botSpawn) ||
        !canPlaceBlock(board, mirror.row, mirror.col, playerSpawn, botSpawn)) continue;

    setBlock(board, item.row, item.col, item.type);
    setBlock(board, mirror.row, mirror.col, item.type);
    placedCells += 2;
  }

  // Final guarantee: always keep a path from player to bot.
