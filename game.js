...
function renderUI() {
  ...
  } else if (state.mode === "online_stub_host" && !state.guestJoined) {
    phaseText.textContent = "Waiting for guest to join...";
  ...
  p2ScoreEl.textContent = String((state.mode === "online_stub_host" && !state.guestJoined) ? 0 : (state.bot ? state.bot.hp : 0));
  p1PosEl.textContent = state.player ? coordLabel(state.player) : "(1,1)";
  p2PosEl.textContent = (state.mode === "online_stub_host" && !state.guestJoined) ? "Waiting..." : (state.bot ? coordLabel(state.bot) : "(8,8)");
  ...
  rollBtn.disabled = !state.mode || state.mode === "online_stub_host" || state.mode === "online_stub_guest" || !(state.phase === "roll" && state.currentTurn === "player" && !state.locked);
  endTurnBtn.disabled = !state.mode || state.mode === "online_stub_host" || state.mode === "online_stub_guest" || !!state.locked || state.phase === "gameOver" || state.phase === "actionChoice";
...
function endTurn() {
  ...
  if (state.mode === "bot" && state.currentTurn === "bot") {
    runBotTurn();
  }
}

async function onTileClick(row, col) {
  if (state.mode !== "bot" || state.currentTurn !== "player" || state.locked || state.phase !== "move") return;
...
rollBtn.addEventListener("click", () => {
  if (state.mode !== "bot" || state.currentTurn !== "player") return;
  rollDice();
});

endTurnBtn.addEventListener("click", async () => {
  if (state.mode !== "bot" || state.phase === "gameOver" || state.locked || state.currentTurn !== "player" || state.phase === "actionChoice") return;
...
document.addEventListener("keydown", (e) => {
  if (state.mode !== "bot" || state.currentTurn !== "player" || state.phase !== "move" || state.locked) return;
  const keyMap = {
...
function createLobbyModal() {
  ...
  backdrop.innerHTML = `
    <div class="action-card lobby-card">
      ...
      <div class="lobby-buttons">
        <button class="action-btn attack" id="playBotBtn">Play with "Aud(weak Bot)win"</button>
        <button class="action-btn heal" id="createGameBtn">Create Game</button>
      </div>

      <div class="lobby-join">
        <input class="lobby-input" id="joinCodeInput" placeholder="Enter password" maxlength="12" />
        <button class="action-btn join" id="joinGameBtn">Join Game</button>
      </div>

      <div class="lobby-note" id="lobbyNote">
        Play with Bot works now. Create Game and Join Game are lobby UI only in this static build.
      </div>
    </div>
  `;
  document.body.appendChild(backdrop);
  ...
  document.getElementById("joinGameBtn").addEventListener("click", () => {
    ...
    if (!/^[A-Z0-9]{5}$/.test(code)) {
      note.textContent = "Enter a 5-character alphanumeric password.";
      return;
    }
    state.mode = "online_stub_guest";
    state.roomCode = code;
    state.guestJoined = true;
...
function ensureRoomPasswordBadge() {
  let badge = document.getElementById("roomPasswordBadge");
  if (!badge) {
    badge = document.createElement("div");
    badge.id = "roomPasswordBadge";
    badge.className = "room-password-badge hidden";
    const topPanel = document.querySelector(".top-panel");
    if (topPanel) topPanel.appendChild(badge);
  }
  return badge;
}

function updateRoomPasswordBadge() {
  const badge = ensureRoomPasswordBadge();
  if (state.roomCode) {
    badge.textContent = `Room Password: ${state.roomCode}`;
...
function shouldRenderUnit(unit) {
  ...
  if (state.mode === "online_stub_host" && !state.guestJoined && unit.team === "bot") return false;
  return true;
}
