...
function hasFrontBush(row, col) {
  return inBounds(row + 1, col)
    && state.board[row + 1][col].blocked
    && state.board[row + 1][col].blockType === "bush";
}
...
function renderTileEngine() {
  ...
  for (let row = 0; row < SIZE; row++) {
    for (let col = 0; col < SIZE; col++) {
      ...
      cell.className = "cell";
      cell.type = "button";
      cell.style.zIndex = String((row + 1) * 10);
      ...
      if (unit) {
        ...
        actor.className = `layer actor ${hasFrontBush(row, col) ? "occluded-by-bush" : ""} ${unit.hitFx ? "target-hit" : ""} ${unit.anim === "hit" ? "player-hit" : ""}`.trim();
