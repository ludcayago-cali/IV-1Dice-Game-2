  ...
  backdrop.innerHTML = `
    <div class="action-card lobby-card">
      ...
      <div class="lobby-buttons">
        <button class="action-btn attack" id="playBotBtn">Play with Bot</button>
      ...
      <div class="lobby-note" id="lobbyNote">
        Online 1v1 is UI-only in this static build. Real guest joining needs a backend or signaling service.
      </div>
    </div>
  `;
  document.body.appendChild(backdrop);
...
showLobbyModal();
  ...
  document.getElementById("createGameBtn").addEventListener("click", () => {
    ...
    document.getElementById("lobbyNote").textContent =
      `Game created. Password: ${code}. Share it with your guest. This build does not include real network sync yet.`;
  });

  document.getElementById("joinGameBtn").addEventListener("click", () => {
    const code = (document.getElementById("joinCodeInput").value || "").trim().toUpperCase();
    if (!code) {
      document.getElementById("lobbyNote").textContent = "Enter a password first.";
      return;
    }
    document.getElementById("lobbyNote").textContent =
      `Join attempt for "${code}" recorded. Real guest joining is not active in this static build yet.`;
  });
}
...
function renderUI() {
  ...
  p1ScoreEl.textContent = String(state.player?.hp ?? 0);
  p2ScoreEl.textContent = String(state.bot?.hp ?? 0);
  p1PosEl.textContent = state.player ? coordLabel(state.player) : "-";
  p2PosEl.textContent = state.bot ? coordLabel(state.bot) : "-";
  ...
  const inGame = !!state.mode;
  rollBtn.disabled = !(inGame && state.phase === "roll" && state.currentTurn === "player" && !state.locked);
  endTurnBtn.disabled = !(inGame && !state.locked && state.currentTurn === "player" && state.phase !== "gameOver" && state.phase !== "actionChoice");
...
async function runBotTurn() {
  if (state.currentTurn !== "bot" || state.winner || state.mode !== "bot") return;
