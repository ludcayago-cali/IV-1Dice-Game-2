...
const state = {
  ...
  lastMessage: "Roll once, then each unit can use that many steps."
};
...
function cloneUnitPositions() {
  return state.units.map(u => ({ id: u.id, alive: u.alive, row: u.row, col: u.col }));
}
...
function startMatch() {
  ...
  state.lastMessage = "Roll once. Each of your 3 units may use up to that many steps.";
...
function renderUI() {
  ...
  turnText.textContent = state.winner ? `Team ${state.winner} Wins` : `Team ${state.turnTeam} Turn`;

  if (state.phase === "preRoll") {
    phaseText.textContent = `Roll once for Team ${state.turnTeam}. That value applies to all 3 units.`;
  } else if (state.phase === "activeTurn") {
    if (!selected) {
      phaseText.textContent = `Select a Team ${state.turnTeam} unit. End Turn is always available.`;
    } else {
      const remaining = Math.max(0, getRemainingStepsForUnit(selected) - state.planningPath.length);
      phaseText.textContent = `${selected.roleName}: used ${selected.usedSteps}/${state.turnSteps} step(s). ${remaining} step(s) left for this unit.`;
    }
  } else {
  ...
  rollBtn.disabled = !(state.phase === "preRoll" && !state.locked);
  endTurnBtn.disabled = !!state.locked || state.phase === "gameOver";
...
function rollDice() {
  ...
  state.planningPath = [];
  state.validMoves = [];
  state.lastMessage = `Team ${state.turnTeam} rolled ${state.turnSteps}. Each unit can use up to ${state.turnSteps} steps.`;
  renderAll();
}
...
function endTurn() {
  ...
  state.turnSteps = null;
  state.locked = false;
  state.lastMessage = `Team ${state.turnTeam}: roll once, then move any or all units.`;
  resetUnitTurnFlags(state.turnTeam);
  renderAll();
}
...
document.addEventListener("keydown", (e) => {
  if (state.phase !== "activeTurn" || state.locked) return;
  const keyMap = {
