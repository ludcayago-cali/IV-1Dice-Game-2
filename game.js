...
const state = {
  ...
  turnCount: 1,
  actionPending: false,
  mode: null,          // null | bot | online_stub_host | online_stub_guest
  roomCode: null,
  guestJoined: false,
};
...
function renderUI() {
  const showBotWaiting = state.mode === "online_stub_host" && !state.guestJoined;
  ...
  p2ScoreEl.textContent = String(showBotWaiting ? 0 : (state.bot ? state.bot.hp : 0));
  p1PosEl.textContent = state.player ? coordLabel(state.player) : "(1,1)";
  p2PosEl.textContent = showBotWaiting ? "Waiting..." : (state.bot ? coordLabel(state.bot) : "(8,8)");
  ...
  const gameplayLocked = !state.mode || showBotWaiting || state.mode === "online_stub_guest";
  rollBtn.disabled = gameplayLocked || !(state.phase === "roll" && state.currentTurn === "player" && !state.locked);
  endTurnBtn.disabled = gameplayLocked || !!state.locked || state.phase === "gameOver" || state.phase === "actionChoice";
...
function shouldRenderUnit(unit) {
  ...
  if (state.mode === "online_stub_host" && !state.guestJoined && unit === state.bot) {
    return false;
  }
  return true;
}
