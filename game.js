...
function reconstructRoute(parent, endKey) {
  const keys = [];
  let cur = endKey;
  while (cur && parent[cur] && parent[cur] !== "START") {
    keys.push(cur);
    cur = parent[cur];
  }
  if (cur && parent[cur] === "START") keys.push(cur);
  keys.reverse();
  return keys.map(k => {
...
function findPathToAnyTarget(startUnit, targetCells, maxSteps) {
  ...
  while (queue.length) {
    ...
    if (!(cur.row === startUnit.row && cur.col === startUnit.col) &&
      ...
      const route = reconstructRoute(parent, curKey);
      return { distance: cur.d, route, destination: { row: cur.row, col: cur.col } };
    }
...
function findProgressRouteTowardEnemies(startUnit, maxSteps) {
  const startKey = keyOf(startUnit.row, startUnit.col);
  const queue = [{ row: startUnit.row, col: startUnit.col, d: 0 }];
  const seen = new Set([startKey]);
  const parent = { [startKey]: "START" };
  ...
  while (queue.length) {
    const cur = queue.shift();
    const curKey = keyOf(cur.row, cur.col);
    const distToEnemy = Math.min(...enemies.map(e => Math.abs(cur.row - e.row) + Math.abs(cur.col - e.col)));
    const progress = Math.min(...enemies.map(e => manhattan(startUnit, e))) - distToEnemy;
    const center = Math.floor(SIZE / 2);
    const centerBias = (SIZE - Math.abs(cur.row - center)) + (SIZE - Math.abs(cur.col - center));
    const score = progress * 100 + centerBias;
    ...
    if (!(cur.row === startUnit.row && cur.col === startUnit.col)) {
      if (!best || score > best.score) {
        best = {
          score,
          distance: cur.d,
          route: reconstructRoute(parent, curKey),
          destination: { row: cur.row, col: cur.col }
        };
      }
    }
...
function chooseBestBotAction() {
  const botUnits = getAliveUnits(BOT_TEAM).filter(u => getRemainingStepsForUnit(u) > 0 || (!u.attackedThisTurn && getAttackTarget(u)));
  ...
  for (const unit of botUnits) {
    ...
    const attackRoute = findPathToAnyTarget(unit, targetCells, steps);
    if (attackRoute && attackRoute.route.length) {
      const enemy = getUnitById(targetCells.find(t => t.row === attackRoute.destination.row && t.col === attackRoute.destination.col)?.enemyId || "");
      let score = 5000 - attackRoute.distance * 100 + scoreBotUnit(unit);
      if (enemy) {
        score += (enemy.hp <= unit.damage ? 1800 : 0);
        score += (12 - enemy.hp) * 25;
      }
      if (!best || score > best.score) {
        best = { type: "moveTowardAttack", unitId: unit.id, route: attackRoute.route, score };
      }
      continue;
    }

    const progressRoute = findProgressRouteTowardEnemies(unit, steps);
    if (progressRoute && progressRoute.route.length) {
      let score = 1200 + progressRoute.score + scoreBotUnit(unit);
      if (!best || score > best.score) {
        best = { type: "moveCloser", unitId: unit.id, route: progressRoute.route, score };
      }
    }
  }
...
async function botSelectAndMove(unit, route) {
  selectUnit(unit);
  await delay(150);
...
async function runBotTurn() {
  ...
  while (state.turnTeam === BOT_TEAM && state.phase === "activeTurn" && !state.winner && safety-- > 0) {
    ...
    if (action.type === "moveTowardAttack" || action.type === "moveCloser") {
      await botSelectAndMove(unit, action.route);
      await delay(180);
      continue;
    }
