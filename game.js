...
function buildBoard(playerSpawn, botSpawn) {
  ...
  const candidates = shuffle(
    allTiles().filter(t => {
      ...
      // keep spawn areas cleaner
      if (Math.abs(t.row - playerSpawn.row) <= 1 && Math.abs(t.col - playerSpawn.col) <= 2) return false;
      if (Math.abs(t.row - botSpawn.row) <= 1 && Math.abs(t.col - botSpawn.col) <= 2) return false;

      // keep a loose center corridor so movement reads better
      if (t.col === 4 || t.col === 5) return false;
      return true;
    })
  );
  ...
  for (const tile of candidates) {
    if (placed >= BLOCK_COUNT) break;

    const type = Math.random() < 0.58 ? "bush" : "crate";

    // asymmetric weighting: more blockers on outer lanes, fewer near center
    const edgeBias = (tile.col <= 2 || tile.col >= SIZE - 3 || tile.row <= 1 || tile.row >= SIZE - 2);
    const allow = edgeBias ? Math.random() < 0.78 : Math.random() < 0.48;
    if (!allow) continue;

    board[tile.row][tile.col].blocked = true;
    board[tile.row][tile.col].blockType = type;

    // keep path guaranteed; revert placements that choke the board
    if (!boardPathExists(board, playerSpawn, botSpawn)) {
      board[tile.row][tile.col].blocked = false;
      board[tile.row][tile.col].blockType = null;
      continue;
    }
...
async function executePlannedPath() {
  ...
  // brief anticipation pause after laying the path
  await delay(220);
